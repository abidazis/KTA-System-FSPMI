            ...
            <tr>
                ...
                <td>
                    <div class="actions">
                        ...
                        @if($user->id !== auth()->id())
                        <form action="{{ route('users.destroy', $user) }}" method="POST" style="display:inline;" id="delete-user-form-{{ $user->id }}">
                            @csrf
                            @method('DELETE')
                            <button type="button" class="btn btn-sm btn-danger" title="Hapus" onclick="FSPMIModal.confirmDelete('Yakin ingin menghapus user ini?').then(function(ok){ if(ok){document.getElementById('delete-user-form-{{ $user->id }}').submit();} });">
